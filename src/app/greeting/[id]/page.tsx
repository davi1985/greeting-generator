...
export default async function GreetingPage({
  ...
  if (!res.ok) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-linear-to-br from-purple-50 to-pink-50 p-6">
        <Card className="max-w-md w-full p-8 text-center shadow-lg">
          <CardContent>
            <div className="text-purple-600 mx-auto mb-4 w-16 h-16 flex items-center justify-center rounded-full bg-purple-100">
            ...
            >
              <a href="/create">Criar nova mensagem</a>
            </Button>
          </CardContent>
        </Card>
      </main>
  ...
  const whatsappShareMessage = `Olá ${name}, hoje é o seu dia! Veja esta mensagem feita pra você:\n${shareUrl}`
  const whatsappShare = `https://wa.me/?text=${encodeURIComponent(
    whatsappShareMessage
  )}`

  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-purple-100 via-pink-50 to-orange-100 py-12 px-4 relative overflow-hidden">
      ...
      <Suspense
        fallback={
          <Card className="p-10 max-w-3xl w-full animate-pulse">
      ...
      <Button
        asChild
        size="icon"
        className="fixed bottom-6 right-6 bg-green-500 hover:bg-green-600 text-white shadow-lg hover:shadow-xl transform hover:scale-110 transition-all duration-300 z-50 rounded-full"
